...
public class SetItemDamage implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String arg, String[] args) {
        if (cmd.getName().equalsIgnoreCase("setitemdamage")) {
            ...
            try {
                ...
                if (itemName.equals("LIST")) {
                    ...
                    for (int i = 0; i < keys.length; i++) {
                        if (keys[i].startsWith("ItemDamage.")) {
                            sender.sendMessage("Item: " + keys[i].substring(11) + " Value: " + plugin.getConfig().getInt("ItemDamage." + keys[i].substring(11)));
                        }
                    }
                    return true;
                }
